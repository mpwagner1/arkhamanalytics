...
def read_file_as_df(
    ...
    try:
        if file_format == "csv":
            ...
            return spark.read.option("header", "true").option("encoding", encoding).csv(file_path)

        elif file_format == "txt":
            logger.info(f"Reading TXT file: {file_path}")
            return spark.read.option("header", "true").option("encoding", encoding).csv(file_path, sep="\t")
